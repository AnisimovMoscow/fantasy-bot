...
class User extends ActiveRecord
{
    ...
    public function saveSettings($settings)
    {
        ...
        if (preg_match('/(\d+)\:(\d+)/', $settings['notificationTime'], $matches)) {
            if (in_array($matches[2], [0, 30])) {
                $this->notification_time = $this->changeTimezone($settings['notificationTime'], $settings['timezone'], Yii::$app->timeZone);
            }
        }
    ...
    private function changeTimezone($time, $fromTimezone, $toTimezone)
    {
